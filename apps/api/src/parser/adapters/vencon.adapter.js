...
var VenconAdapter = /** @class */ (function () {
    function VenconAdapter() {
        this.vendorName = 'vencon.ua';
        this.categoryUrls = {
    ...
    VenconAdapter.prototype.fetchListings = function () {
    ...
    VenconAdapter.prototype.fetchCategory = function (baseUrl_1, rawCategory_1) {
                            ...
                            return __generator(this, function (_b) {
                                switch (_b.label) {
                                    ...
                                    case 2:
                                        html = _b.sent();
                                        $ = cheerio.load(html);
                                        products = $('[class*="product-card"], [class*="catalog-item"]');
                                        if (products.length === 0)
                                            return [2 /*return*/, "break"];
                                        products.each(function (_, el) {
                                            var _a, _b, _c, _d, _e;
                                            var $el = $(el);
                                            var link = $el.find('a[href*="/products/"]').first();
                                            var sourceUrl = (_a = link.attr('href')) !== null && _a !== void 0 ? _a : '';
                                            var rawTitle = link.text().trim() || ((_b = link.attr('title')) === null || _b === void 0 ? void 0 : _b.trim()) || '';
                                            var priceText = $el.find('[class*="price"]').first().text();
                                            var rawPrice = parsePrice(priceText);
                                            var bodyText = $el.text();
                                            var skuText = (_c = bodyText.match(/Код:\s*(\d+)/)) === null || _c === void 0 ? void 0 : _c[1];
                                            // "Закінчується" — товар ещё в наличии, но заканчивается (не outOfStock).
                                            // "Немає в наявності" — явно нет в наличии.
                                            var outOfStock = /немає в наявності/i.test(bodyText);
                                            var image = (_e = (_d = $el.find('img').first().attr('src')) !== null && _d !== void 0 ? _d : $el.find('img').first().attr('data-src')) !== null && _e !== void 0 ? _e : '';
                                            if (!rawTitle || !sourceUrl || rawPrice === null)
                                                return;
                                            listings.push({
                                                sourceUrl: sourceUrl.startsWith('http') ? sourceUrl : "https://vencon.ua".concat(sourceUrl),
                                                sourceSku: skuText,
                                                rawTitle: rawTitle,
    ...
    return VenconAdapter;
}());
exports.VenconAdapter = VenconAdapter;
